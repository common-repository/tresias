<?php
/**
 * This file contains the class WC_Integration_Tiresias_Tagging, that integrates the
 * Tiresias Tagging plugin admin settings page into WooCommerce.
 *
 * @package WooCommerce Tiresias Tagging
 */

/**
 * Tiresias Tagging Integration.
 *
 * This class includes and manages the admin settings page for the plugin.
 * The page is added to "WooCommerce->Settings->Integration->Tiresias Tagging".
 *
 * @since 1.0.0
 */
class WC_Integration_Tiresias_Tagging extends WC_Integration
{
	/**
	 * Default server address for the Tiresias marketing automation service.
	...
	public function __construct() {
		$this->id                 = 'tiresias_tagging';
		$this->method_title       = 'Tiresias Tagging';
		$this->method_description = __( 'Implements the required tagging blocks for using Tiresias marketing automation service.' );

		$this->init_form_fields();
		$this->init_settings();

		add_action( 'woocommerce_update_options_integration_tiresias_tagging', array( $this, 'process_admin_options' ) );
	 ...
	 * - server address (text)
	 * - account id (text)
	 * - use default Tiresias element (radio; yes/no)
	 *
	 * @since 1.0.0
	 */
	function init_form_fields() {
		$this->form_fields = array(
			/*'server_address'       => array(
				...
				'description' => __( 'The server address for the Tiresias marketing automation service.' ),
				'type'        => 'text',
				'default'	  => self::DEFAULT_SERVER_ADDRESS,
			),*/
			'account_id'           => array(
				'title'       => __( 'Account ID' ),
				'description' => __( 'Your Tiresias marketing automation service account id.' ),
				'type'        => 'text',
				'default'	  => '',
			),
			'use_default_elements' => array(
				'title'       => __( 'Use default Tiresias elements' ),
				'description' => __( 'Use default Tiresias elements for showing product recommendations.' ),
				'type'        => 'radio',
				'default'	  => 1,
				'options'     => array(
					array(
	...
	public function register_action_links( $links, $plugin_file ) {
		if ( $plugin_file === WC_Tiresias_Tagging::get_instance()->get_plugin_name() ) {
			$url     = admin_url( 'admin.php?page=wc-setting&tab=integration&section=tiresias_tagging' );
