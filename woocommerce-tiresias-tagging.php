<?php
/*
	Plugin Name: WooCommerce Tiresias Tagging
	Plugin URI: http://wordpress.org/extend/plugins/woocommerce-tiresias-tagging/
	Description: Implements the required tagging blocks for using Tiresias marketing automation service.
	Author: BeTechnology
	Version: 0.0.1
	License: GPLv2
*/

/*	Copyright 2015 BeTechnology  (email : info@betechnology.es)
...
class WC_Tiresias_Tagging
{
	...
	/**
	 * If the default Tiresias elements should be outputted.
	 * This is a setting configured on the admin page.
	 *
	 * @since 1.0.0
	 * @var string
	 ...
	 *
	 * @since 1.0.0
	 * @return WC_Tiresias_Tagging|null
	 */
	public static function get_instance() {
		if ( null === self::$instance ) {
			self::$instance = new WC_Tiresias_Tagging();
		}
	 ...
	 *
	 * Register hooks outputting tagging blocks and Tiresias elements in frontend.
	...
	public function activate() {
		if ( $this->check_dependencies() ) {
			$this->load_class( 'WC_Tiresias_Tagging_Top_Sellers_Page' );
			$page_id = get_option( 'woocommerce_tiresias_tagging_top_sellers_page_id', null );
			$page    = new WC_Tiresias_Tagging_Top_Sellers_Page( $page_id );
			$page->publish();
			if ( null === $page_id ) {
				add_option( 'woocommerce_tiresias_tagging_top_sellers_page_id', $page->get_id() );
			} else {
				update_option( 'woocommerce_tiresias_tagging_top_sellers_page_id', $page->get_id() );
			}
		}
	}
	...
	public function deactivate() {
		$page_id = get_option( 'woocommerce_tiresias_tagging_top_sellers_page_id' );
		if ( $page_id ) {
			$this->load_class( 'WC_Tiresias_Tagging_Top_Sellers_Page' );
			$page = new WC_Tiresias_Tagging_Top_Sellers_Page( $page_id );
			$page->unpublish();
	...
	public static function uninstall() {
		...
		if ( $page_id ) {
			...
			// This is just to avoid duplicating the code in WC_Tiresias_Tagging::load_class().
			WC_Tiresias_Tagging::get_instance()->load_class( 'WC_Tiresias_Tagging_Top_Sellers_Page' );
			$page = new WC_Tiresias_Tagging_Top_Sellers_Page( $page_id );
			$page->remove();
		}

		delete_option( 'woocommerce_tiresias_tagging_settings' );
		delete_option( 'woocommerce_tiresias_tagging_top_sellers_page_id' );
		delete_option( 'widget_tiresias_element' );
	}
	...
	/**
	 * Hook callback function for outputting the Tiresias elements at the bottom of the product page.
	 *
	 * @since 1.0.0
	 */
	public function add_product_page_bottom_elements() {
		if ( is_product() ) {
			$default_element_ids = array(
				'tiresias-page-product1',
				'tiresias-page-product2',
				'tiresias-page-product3',
			);
			...
			if ( is_array( $element_ids ) && ! empty( $element_ids ) ) {
				$this->render( 'tiresias-elements', array( 'element_ids' => $element_ids ) );
			}
		}
	}

	/**
	 * Hook callback function for outputting the Tiresias elements at the top of the category pages.
	 *
	 * @since 1.0.0
	 */
	...
	/**
	 * Hook callback function for outputting the Tiresias elements at the bottom of the category page.
	 *
	 * @since 1.0.0
	 */
	...
	/**
	 * Hook callback function for outputting the Tiresias elements at the bottom of the shopping cart page.
	 *
	 * @since 1.0.0
	 */
	...
	/**
	 * Hook callback function for outputting the Tiresias elements at the top of the search result page.
	 *
	 * @since 1.0.0
	 */
	...
	/**
	 * Hook callback function for outputting the Tiresias elements at the bottom of the search result page.
	 *
	 * @since 1.0.0
	 */
	...
	/**
	 * Hook callback function for outputting the Tiresias elements at the top of all pages.
	 *
	 * @since 1.0.0
	 */
	...
	/**
	 * Hook callback function for outputting the Tiresias elements at the bottom of all pages.
	 *
	 * @since 1.0.0
	 */
	public function add_page_bottom_elements() {
		$default_element_ids = array(
			'tiresias-page-bottom',
		);
	...
	/**
	 * Registers the Tiresias JavaScript to be added to the page head section.
	 *
	...
	public function register_scripts() {
		if ( ! empty( $this->account_id ) ) {
			wp_enqueue_script( 'tiresias-tagging-script', $this->plugin_url . 'js/embed.js' );
			$params = array(
				'accountId' => esc_js( $this->account_id ),
			);
			wp_localize_script( 'tiresias-tagging-script', 'TiresiasTagging', $params );
		}
	}

	/**
	 * Registers widget for showing Tiresias elements in the shop sidebars.
	 *
	 * @since 1.0.0
	 */
	public function register_widgets() {
		$this->load_class( 'WP_Widget_Tiresias_Element' );
		register_widget( 'WP_Widget_Tiresias_Element' );
	}
	...
	/**
	 * Formats price into Tiresias format, e.g. 1000.99.
	 *
	 * @since 1.0.0
	...
	protected function init_admin() {
		$this->load_class( 'WC_Integration_Tiresias_Tagging' );
		add_filter( 'woocommerce_integrations', array( 'WC_Integration_Tiresias_Tagging', 'add_integration' ) );
	}
	...
	protected function init_settings() {
		$settings = get_option( 'woocommerce_tiresias_tagging_settings' );
	...
	protected function check_dependencies() {
		...
		$title = sprintf( __( 'WooCommerce Tiresias Tagging %s not compatible.' ), self::VERSION );
		$error = '';
		$args  = array(
			'back_link' => true,
		);
		...
		if ( version_compare( $wp_version, self::MIN_WP_VERSION, '<' ) ) {
			$error = sprintf(
				__( 'Looks like you\'re running an older version of WordPress, you need to be running at least
					WordPress %1$s to use WooCommerce Tiresias Tagging %2$s.' ),
				self::MIN_WP_VERSION,
				self::VERSION
			);
		}
		...
		if ( ! defined( 'WOOCOMMERCE_VERSION' ) ) {
			$error = sprintf(
				__( 'Looks like you\'re not running any version of WooCommerce, you need to be running at least
					WooCommerce %1$s to use WooCommerce Tiresias Tagging %2$s.' ),
				self::MIN_WC_VERSION,
				self::VERSION
			);
...
add_action( 'plugins_loaded', array( WC_Tiresias_Tagging::get_instance(), 'init' ) );
